...
export interface TocEntry { id: string; level: number; text: string; chap?: boolean }
...
function isFurniture(line: string): boolean {
  ...
  if (new RegExp(`^\\*\\*?\\d{1,2}\\s+(${MONTHS})\\s+\\d{4}\\*?\\*?$`, "i").test(t)) return true;
...
function isJunkHeading(t: string, opts: { title?: string; num?: string }): boolean {
  if (!t) return true;
...
export function parseFM(md: string, opts: { title?: string; num?: string } = {}): ParsedFM {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isLeaderLine(raw)) { flushPara(); continue; }

    const fig = figCaption(t);
    if (fig) { flushPara(); blocks.push({ type: "fig", ...fig }); debris = 1; continue; }
    if (debris > 0) {
      const isHeadingLine = /^#{1,6}\s/.test(raw);
      if (isHeadingLine || isNumberedPara(t) || isBullet(t) || looksSentence(t) || debris > 90) debris = 0;
      else { debris++; continue; }
    }
    ...
    if (hm) {
      ...
      const prevH = blocks.length && blocks[blocks.length - 1].type === "h" ? (blocks[blocks.length - 1] as any) : null;
  ...
  for (let i = 0; i < toc.length; i++) {
    ...
    if (e.chap && nx && !nx.chap) { mergedToc.push({ id: e.id, level: 1, text: e.text + " \u00b7 " + nx.text }); i++; }
