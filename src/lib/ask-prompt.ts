...
export function buildPrompt(question: string, sources: Section[], history: ChatTurn[], mode: AskMode): string {
  ...
  if (mode === "open") {
    return (
      `You are a knowledgeable research assistant with access to a library of U.S. Army Field Manuals (FMs) and curated reference works on organization and management theory. ` +
      `Answer the QUESTION.\n` +
      ...
      `- If you state something that goes beyond the provided excerpts, make clear it is general knowledge and may not match the exact source wording.\n` +
      `- Be concise and authoritative.\n\n` +
      hist + `REFERENCE EXCERPTS (from the library; may be partial):\n${ctx}\n\nQUESTION: ${question}`
    );
  }
  return (
    `You are a research assistant for a library of U.S. Army Field Manuals (FMs) and curated reference works on organization and management theory. ` +
    ...
    `- If the excerpts do not contain the answer, say you could not find it in the indexed sources. Never invent content or cite a source that is not listed.\n` +
